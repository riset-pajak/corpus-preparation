...
def _extract_regulation_meta(soup: BeautifulSoup, fallback_url: str) -> dict[str, str]:
    """Ekstraksi metadata regulasi dari halaman HTML peraturan.go.id."""
    number = ""
    issue_date = ""

    # peraturan.go.id biasanya memiliki struktur metadata terstruktur
    # Cari nomor regulasi
    for selector in [
...
def save_web_regulation(url: str, raw_dir: Path) -> list[DownloadedArtifact]:
    """
    Unduh halaman regulasi dari peraturan.go.id dan semua lampiran PDF yang ditemukan.
