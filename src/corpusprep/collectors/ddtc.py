"""Scraper untuk DDJT (Direktorat Pengawasan Pajak) dan perpajakan.ddtc.co.id.

Sumber daya yang mungkin digunakan:
- Direktur Jenderal Pajak website (pajak.go.id)
- Lampiran Peraturan Direktur Jenderal Pajak
- Situs pengawasan pajak resmi
- perpajakan.ddtc.co.id - Sumber regulasi perpajakan khusus
"""
...
def _extract_regulation_meta(soup: BeautifulSoup, fallback_url: str) -> dict[str, str]:
    """Ekstraksi metadata regulasi dari halaman DPPajak/DJP."""
    number = ""
    issue_date = ""

    # Cari nomor permen/putusan DPPajak
    for selector in [
        ".field--name-field-nomor",
    ...
    for selector in [
        ...
        if node and node.has_attr("datetime"):
            ...
            break
    if not issue_date:
        for selector in [".field--name-field-tanggal", ".tanggal-berlaku"]:
            node = soup.select_one(selector)
            if node:
                issue_date = _normalize_date_text(node.get_text(" ", strip=True))
                break

    # Fallback: ambil dari judul
    if not number:
...
def _number_from_text(value: str) -> str:
    ...
    text = value.upper()
    # Pola standar: PMK-99/2024 or PER-8/PJ/2026
    match = re.search(
...
def _extract_pdf_links(soup: BeautifulSoup, base_url: str) -> list[str]:
    """Ekstrak semua link PDF dari halaman DPPajak."""
    seen: set[str] = set()
    links: list[str] = []
    for anchor in soup.find_all("a", href=True):
        ...
        # Hanya ambil link PDF
        path = urlparse(absolute).path.lower()
        if path.endswith(".pdf") or ".pdf" in path:
            if absolute not in seen:
                seen.add(absolute)
                links.append(absolute)
        # Cek teks anchor untuk lampiran/attachment
        else:
            text = anchor.get_text(" ", strip=True).lower()
            if any(
                kw in text
                for kw in ["pdf", "lampiran", "unduhan", "attachment", "undang-undang"]
            ):
                if absolute not in seen:
                    seen.add(absolute)
                    links.append(absolute)
...
def save_web_regulation(url: str, raw_dir: Path) -> list[DownloadedArtifact]:
    """
    Unduh halaman regulasi dari DJP/DPPajak dan semua lampiran PDF yang ditemukan.

    Strategi:
    - Simpan HTML halaman sebagai sumber asli
    - Simpan PDF lampiran jika ada
    - Hindari overwrite dengan nama file unik
    Sumber ini mencakup permen DPP, keputusan Direktorat, dan peraturan terkait pengawasan pajak.
    """
