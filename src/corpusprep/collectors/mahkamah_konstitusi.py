"""Scraper untuk Mahkamah Konstitusi (mk.go.id)."""
...
def _extract_regulation_meta(soup: BeautifulSoup, fallback_url: str) -> dict[str, str]:
    """Ekstraksi metadata regulasi dari halaman MK."""
    number = ""
    issue_date = ""

    # MK memiliki struktur khusus, cari di berbagai lokasi
    for selector in [
        ".field--name-field-no-urut",
        ".no-urut",
    ...
    # Cari tanggal putusan/putusan
    for selector in [
        ".field--name-field-tanggal-putusan time[datetime]",
        "time[datetime]",
        ".tanggal",
        "[property='dc:date']",
        ".tanggal-putusan",
        ...
        if node and node.has_attr("datetime"):
            ...
            break
    if not issue_date:
        for selector in [".field--name-field-tanggal-putusan", ".tanggal-putusan"]:
            node = soup.select_one(selector)
            if node:
                issue_date = _normalize_date_text(node.get_text(" ", strip=True))
                break
...
def _number_from_text(value: str) -> str:
    ...
    text = value.upper()
    # Pola standar: MK-1/2024 or MK-1/PUU/2024
    match = re.search(
        r"\b((?:MK|MA|UN)?-?\d+(?:/[A-Z0-9.]+)+/\d{4})\b",
        text,
    )
    if match:
...
def _extract_pdf_links(soup: BeautifulSoup, base_url: str) -> list[str]:
    """Ekstrak semua link PDF dari halaman MK."""
    seen: set[str] = set()
    links: list[str] = []
    for anchor in soup.find_all("a", href=True):
        ...
        # Cek teks anchor untuk lampiran
        else:
            text = anchor.get_text(" ", strip=True).lower()
            if "pdf" in text or "lampiran" in text or "undangan" in text:
                if absolute not in seen:
                    seen.add(absolute)
                    links.append(absolute)
    return links
...
def save_web_regulation(url: str, raw_dir: Path) -> list[DownloadedArtifact]:
    """
    Unduh halaman regulasi dari Mahkamah Konstitusi dan semua lampiran PDF yang ditemukan.

    Strategi:
    - Simpan HTML halaman sebagai sumber asli
    - Simpan PDF lampiran jika ada
    - Hindari overwrite dengan nama file unik
    MK typically publishes decisions (putusan) and regulations.
    """
